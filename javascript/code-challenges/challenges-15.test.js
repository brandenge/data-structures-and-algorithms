...
const screenForNames = (arr) => arr.filter(name => /^(Mr\. |Mrs\. |Ms\. |Dr\. )[a-zA-Z][a-zA-Z ]+$/.test(name));
...
const biggerThanLuke = (arr) => {
  let result = arr.filter(char => +char.mass > +arr[0].mass);
  if (result.length === 0) return '';
  result = result.map(char => char.name).join(' - ');
  return result;
};
...
const detectTicTacToeWin = (board) => {
  let result = false;
  if (helpCheck(board, 0, 0, 0, 1, 0, 2)) result = true;
  else if (helpCheck(board, 1, 0, 1, 1, 1, 2)) result = true;
  else if (helpCheck(board, 2, 0, 2, 1, 2, 2)) result = true;
  else if (helpCheck(board, 0, 0, 1, 0, 2, 0)) result = true;
  else if (helpCheck(board, 0, 1, 1, 1, 2, 1)) result = true;
  else if (helpCheck(board, 0, 2, 1, 2, 2, 2)) result = true;
  else if (helpCheck(board, 0, 0, 1, 1, 2, 2)) result = true;
  else if (helpCheck(board, 2, 0, 1, 1, 0, 2)) result = true;
  return result;
};

const helpCheck = (board, row1, col1, row2, col2, row3, col3) => {
  return board[row1][col1] === 'X' && board[row2][col2] === 'X' && board[row3][col3] === 'X' ||
