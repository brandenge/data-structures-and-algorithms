...
const transformToLis = (obj) => Object.keys(obj).map(key => `<li>${key}: ${obj[key]}</li>`);
...
const count = (target, input) => input.flat().filter(n => n === target).length;
...
const totalSum = (input) => input.flat().reduce((sum, n) => sum + n, 0);
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(arr => arr.filter(n => typeof n === 'number' && n % 5 === 0).map(n => Math.pow(2, n)));
};
...
let findShortest = (data) => {
  return data.reduce((shortest, char) => {
    if (+char.height < shortest.height) return { height: char.height, name: char.name };
    else return shortest;
  }, { height: Infinity, name: '' }).name;
};
