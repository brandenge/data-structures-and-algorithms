...
E.g. [4,2,7,5,9,2] -> 9
------------------------------------------------------------------------------------------------ */
const maxInArray = (arr) => arr.reduce((max, n) => n > max ? n : max, -Infinity);
...
const updateNumbers = (obj) => Object.entries(obj).map(person => person.join(': '));
// const updateNumbers = (obj) => Object.keys(obj).map(person => `${person}: ${obj[person]}`);
...
const hasChildrenValues = (arr, character) => {
  const result = arr.find(char => char.name === character);
  return result !== undefined && Object.values(result).some(value => Array.isArray(value));
};

// const hasChildrenValues = (arr, character) => {
//   const result = arr.find(char => char.name === character);
//   return result !== undefined && result.children !== undefined;
// };
...
const hasChildrenEntries = (arr, character) => {
  const result = arr.find(char => char.name === character);
  return result !== undefined && Object.entries(result).flat().includes('children');
};
...
const totalCharacters = (arr) => {
  return arr.map(person => Object.values(person).flat()).flat().filter(name => name).length - arr.length;
};
...
const houseSize = (arr) => {
  ...
  arr.forEach(char => {
    let childCount = 0;
    if (char.children !== undefined) childCount = char.children.length;
    sizes.push({house: char.house, members: childCount + 2});
  });
  return sizes;
};
...
const houseSurvivors = (arr) => {
  ...
  arr.forEach(char => {
    let childCount = 0;
    if (char.children !== undefined) childCount = char.children.length;
    if (deceasedSpouses.includes(char.spouse)) childCount -= 1;
    survivors.push({house: char.house, members: childCount + 2});
  });
  return survivors;
};
