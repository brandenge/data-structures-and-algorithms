...
const listFoods = (recipe) => {
  const result = recipe.ingredients.map(item => {
    ...
    const secondSpaceIndex = item.slice(firstSpaceIndex + 1).indexOf(' ') + firstSpaceIndex + 1;
    return item.slice(secondSpaceIndex + 1);
  });
  return result;
};
...
const splitFoods = (recipe) => recipe.ingredients.map(item => item.split(' ').slice(2).join(' '));
...
const stepActions = (recipe) => recipe.steps.map(step => step.slice(0, step.indexOf(' ')));
...
const totalSumCSV = (str) => str.split(',').reduce((sum, n) => sum + +n, 0);
...
const extractVowels = (str) => {
  return [str.replace(/[aeiou]/g, ''), str.split('').filter(char => 'aeiou'.includes(char)).sort().join('')];
};
