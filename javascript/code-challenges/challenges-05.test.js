...
const addValues = (arr) => arr.reduce((sum, n) => sum + n, 0);
...
const addPurchases = (arr) => arr.reduce((sum, purchase) => sum + purchase.purchasePrice, 0);
...
const countNumberOfElements = (arr) => arr.reduce((length) => length + 1, 0);
...
const reversedString = (str) => str.split('').reduce((reversedStr, char) => char + reversedStr, '');
...
const calculateAverage = (arr) => {
  const result = arr.reduce((acc, n) => {
    return {count: acc.count + 1, sum: acc.sum + n};
  }, {count: 0, sum: 0});
