...
const longestString = (arr) => arr.indexOf(arr.slice().sort((a, b) => b.length - a.length)[0]);
...
const findHappiness = (arr) => arr.filter(str => /:\)/.test(str));
...
const onlyOddChars = (str) => str.split('').filter((c, i) => i % 2 === 1).join('');
...
const allHappy = (arr) => arr.every(str => /:\)/.test(str));
...
const unenrollBrook = (arr) => arr.map(roster => roster.filter(str => !str.includes('Brook')));
...
const sortByDay = (arr) => {
  ...
  daysOfWeek.forEach((day, i) => {
    arr.forEach(event => {
      if (event.includes(day)) result[i].push(event);
    });
  });
  return result;
};
...
const characterByIndex = (arr) => arr.map((char, i) => char[i]);
